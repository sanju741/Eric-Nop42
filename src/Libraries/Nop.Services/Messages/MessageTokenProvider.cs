...
namespace Nop.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        /// <param name="defaultVendorToken"></param>
        /// <returns>HTML table of products</returns>
        protected virtual string ProductListToHtmlTable(Order order, int languageId, int vendorId, bool defaultVendorToken)
        {
            ...
            if (!defaultVendorToken)
                sb.AppendLine($"<th>{_localizationService.GetResource("Messages.Order.Product(s).ManufacturePartNumber", languageId)}</th>");
            ...
            for (var i = 0; i <= table.Count - 1; i++)
            {
                ...
                //SKU
                if (_catalogSettings.ShowSkuOnProductDetailsPage && defaultVendorToken)
                {
                ...
                if (defaultVendorToken)
                {
                    ...
                    if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
                    {
                        ...
                        unitPriceStr = _priceFormatter.FormatPrice(unitPriceInclTaxInCustomerCurrency, true,
                            order.CustomerCurrencyCode, language, true);
                    }
                    else
                    {
                        //excluding tax
                        ...
                        unitPriceStr = _priceFormatter.FormatPrice(unitPriceExclTaxInCustomerCurrency, true,
                            order.CustomerCurrencyCode, language, false);
                    }
                ...
                if (defaultVendorToken)
                {
                    ...
                    if (order.CustomerTaxDisplayType == TaxDisplayType.IncludingTax)
                    {
                        ...
                        priceStr = _priceFormatter.FormatPrice(priceInclTaxInCustomerCurrency, true,
                            order.CustomerCurrencyCode, language, true);
                    }
                    else
                    {
                        ...
                        priceStr = _priceFormatter.FormatPrice(priceExclTaxInCustomerCurrency, true,
                            order.CustomerCurrencyCode, language, false);
                    }
                ...
                if (!defaultVendorToken)
                {
                    sb.AppendLine($"<td style=\"padding: 0.6em 0.4em;text-align: center;\">{product.ManufacturerPartNumber}</td>");
                }
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, bool defaultVendorToken, int vendorId = 0)
        {
            ...
            tokens.Add(new Token("Order.Product(s)", ProductListToHtmlTable(order, languageId, vendorId, defaultVendorToken), true));
